...
impl Workbook {
                ...
                for cell in row.cells.values_mut() {
                    ...
                    if existing_index.is_none() {
                        ...
                        index.try_reserve(existing_table_len).map_err(|source| {
                            err_with_source(
                                format!(
                                    "기존 shared string index map 메모리 확보 실패: {existing_table_len} entries"
                                ),
                                source,
                            )
                        })?;
                    ...
                    let Some(existing_index_ref) = existing_index.as_ref() else {
                        return Err(err("기존 shared string index 상태가 비정상적입니다."));
                    };
    ...
    pub fn update_filter_database_defined_name(
        ...
        quoted_sheet
            .try_reserve(quoted_sheet_capacity)
            .map_err(|source| err(format!("시트 이름 quote 메모리 확보 실패: {source}")))?;
        quoted_sheet.push('\'');
        ...
        plain_sheet
            .try_reserve(plain_sheet_capacity)
            .map_err(|source| err(format!("시트 이름 prefix 메모리 확보 실패: {source}")))?;
        plain_sheet.push_str(sheet_name);
        plain_sheet.push('!');
        let sheet_idx = super::workbook_sheet_index_by_name(&self.xml_text, sheet_name)?;
        let end_col = col_to_name(last_filter_col)?;
        ...
        replacement
            .try_reserve(replacement_capacity)
            .map_err(|source| err(format!("_FilterDatabase ref 메모리 확보 실패: {source}")))?;
        replacement.push_str(&quoted_sheet);
        replacement.push_str("$A$");
        FmtWrite::write_fmt(&mut replacement, format_args!("{header_row}"))
            .map_err(|source| err(format!("_FilterDatabase ref 작성 실패: {source}")))?;
        replacement.push_str(":$");
        replacement.push_str(&end_col);
        replacement.push('$');
        FmtWrite::write_fmt(&mut replacement, format_args!("{last_data_row}"))
            .map_err(|source| err(format!("_FilterDatabase ref 작성 실패: {source}")))?;
    ...
    pub(super) fn verify_sheet_address_data_end_row(
        ...
        for col in worksheet.cols_in_row(header_row) {
            ...
            key.try_reserve(display.len()).map_err(|source| {
                err_with_source(
    ...
    fn parse(&self) -> Result<ParsedWorksheetRows> {
        ...
        while let Some(row_info) = scanner.next_start_named("row") {
            ...
            let row_num = if let Some(row_num_text) = get_attr(&row_attrs, "r") {
                let parsed = row_num_text
                    .parse::<u32>()
                    .map_err(|source| err_with_source("worksheet row 번호 해석 실패", source))?;
                if parsed == 0 {
                ...
                rows.keys()
                    .next_back()
                    .copied()
                    .map_or(Ok(1_u32), |last_row| {
                        last_row.checked_add(1).ok_or_else(|| {
...
impl WorksheetXmlParser<'_> {
    fn parse(&self) -> Result<Worksheet> {
        ...
        if sheet_data_tag.self_closing {
            let sheet_data_name = sheet_data_tag.name;
            let open_tag = self
                .xml
                .get(RangeInclusive {
            ...
            let sheet_data_end =
                checked_usize_add(sheet_data_open_end, 1, "worksheet sheetData 끝")?;
            ...
            prefix.try_reserve(prefix_capacity).map_err(|source| {
            ...
            suffix.try_reserve(suffix_capacity).map_err(|source| {
...
impl Worksheet {
    pub fn clear_cells_in_rows_through_col(&mut self, rows: RangeInclusive<u32>, max_col: u32) {
        ...
        self.formula_cells
            .retain(|&(row, col)| row < clear_start || row > clear_last || col > max_col);
    }
    pub fn clear_formula_cached_values_in_range<R>(&mut self, rows: R) -> Result<()>
    where
        R: RangeBounds<u32>,
    {
        let start_bound = rows.start_bound();
        let end_bound = rows.end_bound();
        let rows_map = &mut self.rows;
        let formula_cells = &mut self.formula_cells;
        let mut result = Ok(());
        formula_cells.retain(|&(row_num, col)| {
            if result.is_err() {
                return true;
            }
            if match start_bound {
                Bound::Included(row) => row_num < *row,
                Bound::Excluded(row) => row_num <= *row,
    ...
    pub fn copy_row_style(&mut self, source_row: u32, target_row: u32, max_col: u32) -> Result<()> {
        ...
        copied.cells.retain(|col, _| *col <= max_col);
    ...
    pub fn extend_conditional_formats(
        ...
        while let Some(location) = find_start_tag_location(
            ...
            let updated_sqref = maybe_updated_sqref.map_or(sqref, |updated_sqref| updated_sqref);
        ...
        for (row_num, row) in &mut self.rows {
            for (col_num, cell) in &mut row.cells {
                ...
                let formula = if let Some(text) = spec.formula_text {
                    ...
                    let delta_col = i64::from(*col_num)
                        .checked_sub(i64::from(head.anchor_col))
                        .ok_or_else(|| err("shared formula column delta 계산 실패"))?;
                    let delta_row = i64::from(*row_num)
                        .checked_sub(i64::from(head.anchor_row))
    ...
    fn prune_col_definitions_after_col(&mut self, max_col: u32) -> Result<()> {
            ...
            find_start_tag_location(&self.prefix, "col", cursor, "col 정의 태그 이름 복사")?
        {
            ...
            let min_col = get_attr(&attrs, "min")
                .ok_or_else(|| err("worksheet col 정의에 min 속성이 없습니다."))?
                .parse::<u32>()
                .map_err(|source| err_with_source("worksheet col min 해석 실패", source))?;
            let max_defined_col = get_attr(&attrs, "max")
                .ok_or_else(|| err("worksheet col 정의에 max 속성이 없습니다."))?
                .parse::<u32>()
                .map_err(|source| err_with_source("worksheet col max 해석 실패", source))?;
            if min_col > max_col {
            ...
            if max_defined_col > max_col {
                set_attr(&mut attrs, "max", display_string(max_col));
    ...
    pub fn set_formula_cached_value_at(
        ...
        let encoded = value
            ...
            .transpose()?;
        let value_text = encoded.as_deref().map_or("", |text| text);
    ...
    pub fn try_get_display_at<'text>(
        ...
        let inner = cell.inner_xml.as_deref().map_or("", |xml| xml);
        match cell_type {
            ...
            ordinary_type => match ordinary_type {
                Some("s") => {
                    ...
                    let idx = raw_v.parse::<usize>().map_err(|source| {
                ...
                _ => {
                    let raw_v = extract_first_tag_text(inner, "v")?.map_or("", |text| text);
    ...
    pub fn update_dimension(&mut self) -> Result<()> {
        ...
        for (&row_num, row) in &self.rows {
            if let Some(&col) = row.cells.keys().next_back() {
...
fn copy_attrs(attrs: &[XmlAttr], context: &'static str) -> Result<Vec<XmlAttr>> {
    ...
    for attr in attrs {
        out.push(XmlAttr {
            name: copy_text(&attr.name, context)?,
...
fn usize_attr_or(
    ...
    get_attr(attrs, name).map_or(Ok(default), |value| {
        value
            .parse::<usize>()
...
fn append_nested_text_xml(
    ...
    ensure_valid_xml_text(text, context)?;
    let escaped_capacity = text
        .len()
        .checked_mul(6)
        .ok_or_else(|| err(format!("{context} escape 용량 계산 실패")))?;
...
fn append_peer_text_tag(
    ...
    xml.try_reserve(capacity).map_err(|source| {
        err_with_source(
...
fn replace_first_tag_text(xml: &mut String, tag_name: &str, new_text: &str) -> Result<bool> {
    ...
    if trimmed_open_tag.ends_with("/>") {
        ...
        let mut replacement = String::new();
        replacement.try_reserve(capacity).map_err(|source| {
            err_with_source(
...
fn build_display_text_tag(name: &str, value: impl Display) -> Result<String> {
    let tag_name_len = name
        .len()
        .checked_mul(2)
        .ok_or_else(|| err("XML 표시값 태그 이름 용량 계산 실패"))?;
    let capacity = checked_capacity(&["<></>".len(), tag_name_len, MAX_DECIMAL_TEXT_LEN])
    ...
    out.push_str(name);
    out.push('>');
    FmtWrite::write_fmt(&mut out, format_args!("{value}"))
        .map_err(|fmt_err| err(format!("XML 표시값 태그 쓰기 실패: {fmt_err}")))?;
    out.push_str("</");
    out.push_str(name);
    out.push('>');
    Ok(out)
...
fn row_only_error(prefix: &str, row_num: u32) -> String {
    ...
    let Some(capacity) = checked_capacity(&[prefix.len(), row_text.len(), 1]) else {
