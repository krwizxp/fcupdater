...
- Excel 미설치 환경에서 동작
- Opinet 전국 단일 소스 파일 자동 다운로드(Windows 기본)
- `.xls` / `.xlsx` 소스 입력 지원
- 주소 기반 매칭으로 기존 행 갱신
...
- Windows 자동 다운로드: 브라우저/WebDriver 없이 Windows WinHTTP 사용
- macOS/Linux: 자동 다운로드는 지원하지 않으며 `--skip-download`로 기존 소스 파일 사용
- 압축/해제 도구
...
fcupdater.exe
```
3. 실행 중 Opinet에서 전국 현재 판매가격 파일 1건을 자동 다운로드하고, 11개 대상 지역만 필터링해 현행화를 진행합니다. 기본 소스 prefix는 `현재 판매가격(주유소)`입니다.
4. 기본 출력 파일:
...
- 자동 다운로드는 Windows에서 WinHTTP로 수행하며, 브라우저나 WebDriver를 실행하지 않습니다.
- macOS/Linux에서는 `--skip-download`로 기존 소스 파일을 지정해 현행화할 수 있습니다.
